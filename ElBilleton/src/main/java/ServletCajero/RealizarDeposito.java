...
public class RealizarDeposito extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Transaccion transaccionDeposito = new Transaccion(Long.valueOf(0), Date.valueOf(LocalDate.now()), Time.valueOf(LocalTime.now()), "CREDITO", Double.parseDouble(monto), cajero.getCodigo(), cuenta1.getCodigo());
            transaccion.agregartransaccion2(transaccionDeposito);
            cuenmodel.modificarMonto(cuenta1);
            request.getRequestDispatcher("/Cajero/Mensaje.jsp?mensaje=Deposito realizado con exito").forward(request, response);

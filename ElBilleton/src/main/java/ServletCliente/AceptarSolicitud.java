    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
       ...
       int codigoSolicitud = Integer.valueOf((String) request.getParameter("solicitud"));
        ...
        try {
            ...
            Solicitud solicitud = solicitudModel.obtenerSolicitudCodigo(codigoSolicitud);
            comprobacion = modeloAsociacion.comprobarAsociacion(solicitud.getCuentaEnvio(), solicitud.getCuentaRecibe());

            if (comprobacion == false) {
                solicitud.setEstado("Aceptada");
                solicitudModel.actualizarAsociacion(solicitud, codigoSolicitud);
                Date fecha = Date.valueOf(LocalDate.now());
                Asociacion asociacion = new Asociacion(0, fecha, codigoSolicitud);
                modeloAsociacion.crearAsociacion(asociacion);
                request.getRequestDispatcher("/Cliente/Mensaje.jsp?mensaje=Asociacion aceptada con exito").forward(request, response);
            } else if (comprobacion == true) {

               request.getRequestDispatcher("/Cliente/Mensaje.jsp?mensaje=La asociacion ya existe").forward(request, response);
                solicitud.setEstado("Aceptada");
                solicitudModel.actualizarAsociacion(solicitud, codigoSolicitud);
            }
